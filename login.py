...
from PIL import Image, ImageTk
# We still need Tk and Label from tkinter for the window and image labels
...
class login:
    def __init__(self, root):
        ...
        # Width and height are now specified here
        login_frame = ctk.CTkFrame(
            self.root, width=425, height=262, corner_radius=100, fg_color="white")
        login_frame.place(x=966, y=369.51)

        # Labels and Entries (CTkLabel and CTkEntry for a modern look)

        self.username_entry = ctk.CTkEntry(login_frame, corner_radius=100, fg_color="white", width=325,
                                           # Rounded corners for the entry field
                                           height=40, placeholder_text="username", text_color="#3D3D3D")
        self.username_entry.grid(row=0, column=1, padx=10, pady=10)

        self.password_entry = ctk.CTkEntry(login_frame, corner_radius=100, show="*", fg_color="white",
                                           width=325, height=40, placeholder_text="password", text_color="#3D3D3D")  # Rounded corners
        self.password_entry.grid(row=1, column=1, padx=10, pady=10)

        # Login button (CTkButton for a modern button look)
        login_button = ctk.CTkButton(login_frame, text="Login", corner_radius=100, command=self.validate_login,
                                     fg_color="#31C975", text_color="white", width=325, height=40, font=('lato bold', 20))
    ...
    def load_and_display_images(self):
        ...
        signup_button = ctk.CTkButton(self.root, text="Signup", corner_radius=100, fg_color="#4383EC",
                                      text_color="white", width=460, height=50, font=('lato bold', 20), hover_color="#31C975")
        signup_button.place(x=205, y=581)
        # signup_button.place(x=205, y=581)

        # Image-3

        # login_right  # Path of image
        img3 = Image.open(r"Image\login_right.png")
...
if __name__ == "__main__":
    ...
    root = Tk()
    obj = login(root)  # Corrected class name
    root.mainloop()
